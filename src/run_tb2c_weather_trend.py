# -*- coding: utf-8 -*-
"""Tier B — TB2c: 날씨 실측, origin 기준 최근 7일 추세(배포 가능 버전). run_tb2b_weather_gated.py와
동일한 저울, 후보만 다르다.

📌 사전등록 (experiments/log_tierb.md TB2c 절)
    가설: TB2/TB2b는 예측 대상일(td)의 실측 날씨라 "완벽예보가 있다면"의 상한선 실험이었다.
    origin(창이 끝나는, 이미 지난 시점) 기준 최근 7일 실측(ta_avg 평균·rn_day 합·sd_max 최댓값)으로
    바꾸면 배포 가능하면서도 "올해 실제로 봄/겨울이 얼마나 진행됐는지"라는 같은 정보를 준다.
    게이팅(화담숲=개장기간만·느티나무=연중)은 TB2b와 동일.
...
GROUP = "tb_weather_trend"
CAND_KEYS = F.TB_WEATHER_TREND_KEYS
ARMS = [("L0 base(54)", []), (f"TB2c +weather_trend({54 + len(CAND_KEYS)})", CAND_KEYS)]
CANDIDATE = ARMS[1][0]
...
OUT = os.path.join(C.EXPERIMENTS, "phase_tb2c_weather_trend.json")
