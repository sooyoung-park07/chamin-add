# -*- coding: utf-8 -*-
"""Tier B — TB7b: "곤지암 스키장" 검색 모멘텀 + 업장 게이트 + **계절 게이트**(신규).
run_tb7_naver_ski.py와 원본 피처는 동일(척도 수정 + 3일창 재설계된 naver_ski_last3.csv), td가
스키 시즌이 아니면 값을 강제로 0으로 끊는 계절 게이트만 추가.

📌 사전등록 (experiments/log_tierb.md TB7b 절, 결과 보기 전에 기록 — 2026-08-15)
    게이트: prod 평균 > +0.0015 AND prod 4/4 일관 AND raw 4/4 양수.
    TB7(척도 수정판)과 나란히 비교해서 계절 게이팅의 순수 효과를 가른다.
"""
import os
import json
import time
...
GROUP = "tb_naver_ski_season"
CAND_KEYS = ["naver_ski_last3_season_g"]
ARMS = [("L0 base(54)", []), (f"TB7b +naver_ski_season({54 + len(CAND_KEYS)})", CAND_KEYS)]
CANDIDATE = ARMS[1][0]
OUT = os.path.join(C.EXPERIMENTS, "phase_tb7b_naver_ski_season.json")
...
def main():
    ...
    print("\n" + "=" * 90)
    for tag, nm in (("prod", "실제 파이프라인 (seg+스냅)"), ("raw", "raw (하한만) - 규칙 19")):
