# -*- coding: utf-8 -*-
"""Tier B — TB2d: 날씨 실측, origin 기준 최근 7일 추세 · 느티나무 제외(화담숲만).
run_tb2c_weather_trend.py와 동일한 저울, 게이트(화담숲만 남긴다)만 다르다.

📌 사전등록 (experiments/log_tierb.md TB2d 절)
    가설: TB2/TB2b/TB2c 셋 다 FAR-겨울(학습 데이터 최소 폴드)에서 손해를 봤는데, 공통점은
    겨울엔 화담숲 게이트가 꺼지고 느티나무 단독 신호만 남는다는 것이었다. 느티나무를 완전히
    빼고 화담숲(개장기간만)만 남기면 겨울엔 항상 0이 되어 그 잡음원이 구조적으로 사라진다.
...
GROUP = "tb_weather_trend_hw"
CAND_KEYS = F.TB_WEATHER_HWADAM_KEYS
ARMS = [("L0 base(54)", []), (f"TB2d +weather_hwadam({54 + len(CAND_KEYS)})", CAND_KEYS)]
CANDIDATE = ARMS[1][0]
...
OUT = os.path.join(C.EXPERIMENTS, "phase_tb2d_weather_hwadam.json")
