# -*- coding: utf-8 -*-
"""Tier B — TB2b: TB2(기상청 실측 날씨) 재시도, 업장군×개장기간 게이트.
run_tb2_weather.py와 동일한 저울. 바뀌는 것은 CAND_KEYS와 그룹뿐이다.

📌 사전등록 (experiments/log_tierb.md TB2b 절)
    가설: TB2는 겨울 2폴드 손해·봄 2폴드 이득으로 4/4 게이트 탈락(전 업장에 동일 값 tile).
    화담숲(개장기간만)·느티나무(연중) 업장군에만 값을 주면 겨울 폴드의 손해가 사라져
    4/4 일관을 달성한다. 게이트 경계는 TB2 폴드 결과가 아니라 기존 HWADAM_OPEN 상수로 고정.
    예상치: prod 평균 +0.005 ~ +0.015.
...
GROUP = "tb_weather_v2"
CAND_KEYS = F.TB_WEATHER_GATED_KEYS
ARMS = [("L0 base(54)", []), (f"TB2b +weather_gated({54 + len(CAND_KEYS)})", CAND_KEYS)]
CANDIDATE = ARMS[1][0]
...
OUT = os.path.join(C.EXPERIMENTS, "phase_tb2b_weather_gated.json")
