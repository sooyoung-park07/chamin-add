# -*- coding: utf-8 -*-
"""Tier B — TB2: 기상청 실측 날씨(이천 ASOS). run_tb1_hwadam_actual.py와 동일한 저울.

📌 사전등록 (experiments/log_tierb.md TB2 절)
    가설: 예측 대상일의 실측 기온/강수/적설(wthr_ta_avg·wthr_rn_day·wthr_sd_max)이
    화담숲·느티나무·스키 계열처럼 날씨 민감 업장의 예측을 개선한다.
    ⚠️ 이건 예측 대상일의 "실제" 날씨라 완벽한 예보가 있다는 가정의 상한선 실험이다.
...
GROUP = "tb_weather"
CAND_KEYS = F.TB_WEATHER_KEYS
ARMS = [("L0 base(54)", []), (f"TB2 +weather({54 + len(CAND_KEYS)})", CAND_KEYS)]
CANDIDATE = ARMS[1][0]
...
OUT = os.path.join(C.EXPERIMENTS, "phase_tb2_weather.json")
