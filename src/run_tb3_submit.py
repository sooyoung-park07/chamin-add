# -*- coding: utf-8 -*-
"""Tier B — TB3 채택 확인 제출본: v24(54피처) + 관광공사 방문자수 3종(tb_visit) = 57피처 전체학습.

`run_phase18_prune_submit.py`와 동일한 절차(시드5·라운드1000·L1)에 tb_visit만 더한다.
⚠️ Tier B — 대회 규정 밖(외부데이터). v24 계열 번호와 분리, `x1` 라벨을 쓴다(README/AGENTS.md 규약).
"""
import os
import time
...
ROUNDS = 1000
OUT = os.path.join(C.EXPERIMENTS, "phase_tb3_visit_x1_raw.npy")
...
def main():
    ...
    print(f"피처 {len(names)}개 (v24 54 + tb_visit 3): {', '.join(F.TB_VISIT_KEYS)}", flush=True)
    ...
    print("다음: python make_submission.py x1_visit --raw "
