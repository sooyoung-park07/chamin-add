# -*- coding: utf-8 -*-
"""Tier B — TB3: 한국관광공사 지역방문자수(경기 광주시). run_tb1_hwadam_actual.py와 동일한 저울.

📌 사전등록 (experiments/log_tierb.md TB3 절)
    가설: 예측 대상일의 실측 방문자수(현지인/외지인/외국인, 경기 광주시 단위)가 창(28일)이
    구조적으로 못 보는 "그 주에 사람이 얼마나 몰렸는가" 정보를 보태 예측을 개선한다.
    ⚠️ 이건 예측 대상일의 "실제" 방문자수라 완벽한 예측치가 있다는 가정의 상한선 실험이다.
...
GROUP = "tb_ski_close"
CAND_KEYS = ["d_to_ski_close_actual"]
ARMS = [("L0 base(54)", []), (f"TB6 +ski_close_actual({54 + len(CAND_KEYS)})", CAND_KEYS)]
CANDIDATE = ARMS[1][0]
OUT = os.path.join(C.EXPERIMENTS, "phase_tb6_ski_close_actual.json")
