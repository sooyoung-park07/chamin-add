# -*- coding: utf-8 -*-
"""Tier B — TB2e: 날씨 실측, origin 기준 최근 7일 추세 · 화담숲·느티나무 독립 열로 분리.
run_tb2d_weather_hwadam.py와 동일한 저울, 후보(느티나무를 별도 열로 되살림)만 다르다.

📌 사전등록 (experiments/log_tierb.md TB2e 절)
    가설: TB2d(화담숲 단독)에서도 FAR-겨울 손해(-0.00517)가 남아있었다 — 진단 결과
    그 구간에서 화담숲 게이트가 켜지는 날은 6.59%뿐이라, "느티나무와 섞여서"가 아니라
    열 추가 자체가 feature_fraction 표본추출을 흔든 부작용(규칙 ⑯)일 가능성이 크다.
    그래도 TB2d가 느티나무를 통째로 지우며 F2의 실신호까지 버린 손실은 되살릴 수 있을지
    시험한다 — 화담숲·느티나무를 하나의 게이트 값으로 묶지 않고 완전히 독립된 열로 분리.
    FAR-겨울 손해는 안 고쳐질 것으로 예상하되(진단상 열 자체 부작용), F2는 TB2c 수준
    (+0.00095)으로 회복할지가 관전 포인트.
...
GROUP = "tb_weather_split"
CAND_KEYS = F.TB_WEATHER_HWADAM_KEYS + F.TB_WEATHER_GREEN_KEYS
ARMS = [("L0 base(54)", []), (f"TB2e +weather_split({54 + len(CAND_KEYS)})", CAND_KEYS)]
CANDIDATE = ARMS[1][0]
...
OUT = os.path.join(C.EXPERIMENTS, "phase_tb2e_weather_split.json")
