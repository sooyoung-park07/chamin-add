# -*- coding: utf-8 -*-
"""Tier B — TB1: 화담숲 실측 개장일. 대회 규정 밖(외부 뉴스 조사)이므로 v24 계열과 분리.

`run_phase29_ramp.py`와 동일한 저울(4폴드·짝시드5·prod/raw 동시 채점·사전등록 게이트)을
그대로 재사용한다. 후보는 단 하나: v24 베이스(54피처)에 `d_to_hwadam_open_actual` 1개만 추가.

📌 사전등록 (experiments/log_tierb.md TB1 절 참고 — 결과 보기 전 고정)
    가설: 연도별 실측 개장일(2023-03-31·2024-03-29·2025-03-28, 뉴스 교차확인)로
    개장까지 거리를 다시 계산하면 화담숲 관련 품목의 봄 전환기 예측이 개선된다.
...
DROP3 = ["w_posmedian", "w_last14", "w_std"]      # v24 확정 구성(54피처)과 동일하게 뺀다

ARMS = [("L0 base(54)", []),
        ("TB1 +open_actual(55)", F.TB_RAMP_KEYS)]
CANDIDATE = "TB1 +open_actual(55)"
...
OUT = os.path.join(C.EXPERIMENTS, "phase_tb1_hwadam_actual.json")
...
def main():
    ...
    keep_all = F.active_columns(include=("tb_ramp",))
    names_all = F.active_names(include=("tb_ramp",))
    base_sub = [i for i, n in enumerate(names_all)
                if n not in DROP3 and n not in F.TB_RAMP_KEYS]
    print(f"전체 활성 {len(names_all)}개 · base {len(base_sub)}개 "
          f"(= v24 54개인가: {len(base_sub) == 54})", flush=True)

    smp = [pd.Timestamp("2023-03-30"), pd.Timestamp("2024-03-28"),
           pd.Timestamp("2025-03-27")]
    print("  실측 vs 고정 개장거리 점검:")
    for d in smp:
        va, vf = F._signed_days_to_actual(d), F._signed_days_to(d, *F.HWADAM_OPEN_MD)
        print(f"    {d.date()}  실측 {va:+.0f}일 · 고정 {vf:+.0f}일", flush=True)
    ...
    for fname, cut, v0, v1 in FOLDS:
        ...
        print(f"  [{fname:<8s}] base prod {b['prod']:.5f} · raw {b['raw']:.5f}"
              f" · TB1 prod {b['prod']-c['prod']:+.5f} · raw {b['raw']-c['raw']:+.5f}"
