# -*- coding: utf-8 -*-
"""Tier B — TB7b 리더보드 확인용 제출본: v24(54피처) + naver_ski_last3_season_g(TB7b,
ski 업장군 + 계절 게이팅) = 55피처 전체학습. run_tb7_submit.py와 동일 절차.

⚠️ 내부 4폴드 게이트 미달(log_tierb.md TB7b절 — prod 평균 -0.00105, 일관 2/4,
TB7보다도 악화). "혹시 모르니" 확인용.
"""
import os
import time
...
OUT = os.path.join(C.EXPERIMENTS, "phase_tb7b_naver_ski_season_x4_raw.npy")
...
def main():
    ...
    print(f"피처 {len(names)}개 (v24 54 + {GROUP} {len(F.TB_NAVER_SKI_SEASON_KEYS)}): "
          f"{', '.join(F.TB_NAVER_SKI_SEASON_KEYS)}", flush=True)
    ...
    print("다음: python src/make_submission.py x4_naver_ski_season --raw "
