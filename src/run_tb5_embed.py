# -*- coding: utf-8 -*-
"""Tier B — TB5: 메뉴명 OpenAI 임베딩 최근접 품목. run_tb1_hwadam_actual.py와 동일한 저울.

📌 사전등록 (experiments/log_tierb.md TB5 절)
    가설: 의미상 가장 가까운 품목(emb_sim_last7·emb_sim_dow, x_proxy_*와 동일 설계)의 최근
    판매량이 특히 희귀 품목의 예측을 개선한다.
...
GROUP = "tb_embed"
CAND_KEYS = F.TB_EMBED_KEYS
ARMS = [("L0 base(54)", []), (f"TB5 +embed({54 + len(CAND_KEYS)})", CAND_KEYS)]
CANDIDATE = ARMS[1][0]
...
OUT = os.path.join(C.EXPERIMENTS, "phase_tb5_embed.json")
